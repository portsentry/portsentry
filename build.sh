...
if [ "$ACTION" = "clean" ]; then
  ...
  docker buildx build -t portsentry:unstable -f docker/Dockerfile --platform=linux/amd64,linux/arm64,linux/arm/v7,linux/arm/v6,linux/i386 .
elif [ "$ACTION" = "docker_export" ]; then
  BUILD_DIR=/tmp/portsentry-build
  rm -rf $BUILD_DIR
  docker buildx build -t export -f docker/Dockerfile --target export --platform=linux/amd64,linux/arm64,linux/arm/v7,linux/arm/v6,linux/i386 --output type=local,dest=$BUILD_DIR .
